import { fetchCid, fetchJsonCid, getCommitTreeCid, nodeIsTree } from "./index"
...
export async function getBlob({ cid, path }) {
  ...
  const blobCid = navToSubtree(cache.dirStructure, path);
...
function navToSubtree(tree, pathArray) {
  let subTree = tree;
